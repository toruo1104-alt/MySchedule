...
- **フェーズ5(Cloudflare D1+Workers移行・ハイブリッド構成)を `phase5-db` ブランチに実装済み**(2026-08-01。敵対的レビュー2本+E2Eリハーサル済み)。設計=`docs/DB設計.md`、手順=`docs/構築手順書.md`。**カットオーバー待ち: ユーザーのCloudflareアカウント作成+wrangler loginが起点**。mainへのマージはWorker並行稼働の確認後(マージ時点で旧Pages URLは404になる。手順書§6)
